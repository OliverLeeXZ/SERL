...
SAMPLING_MODE=${SAMPLING_MODE:-env_feedback}
...
COMMON_ARGS=(
    ...
    "actor_rollout_ref.actor.policy_loss.loss_mode=rlsd_action_mask"
    ...
    "actor_rollout_ref.actor.rlsd.sampling_mode=${SAMPLING_MODE}"
    "actor_rollout_ref.actor.rlsd.trajectory_format=${TRAJECTORY_FORMAT}"
    "actor_rollout_ref.actor.rlsd.mixing_lambda=${MIXING_LAMBDA:-0.5}"
    "actor_rollout_ref.actor.rlsd.lambda_decay_steps=${LAMBDA_DECAY_STEPS:-50}"
    "actor_rollout_ref.actor.rlsd.weight_clip=${WEIGHT_CLIP:-0.2}"
    "actor_rollout_ref.actor.rlsd.teacher_sync_interval=${TEACHER_SYNC_INTERVAL:-10}"
    "actor_rollout_ref.actor.rlsd.judge_api_url=${JUDGE_API_URL}"
    "actor_rollout_ref.actor.rlsd.judge_model=${JUDGE_MODEL}"
    "actor_rollout_ref.actor.rlsd.judge_api_key=${JUDGE_API_KEY}"
    "actor_rollout_ref.rollout.name=${ENGINE}"
    "actor_rollout_ref.rollout.mode=sync"
    "actor_rollout_ref.rollout.log_prob_micro_batch_size_per_gpu=${ROLLOUT_LOG_PROB_MICRO_BATCH_SIZE_PER_GPU:-8}"
...
python3 -m recipe.rlsd.main_rlsd "${COMMON_ARGS[@]}" "$@"
